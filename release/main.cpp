...
//
// test03
// This is a trivial test that i) checks if the NumVertices() function
...
TEST(testing, test04)
{
    ...
    bool added;
    // adding vertices
    added=g.addVertex("insert text");
    ASSERT_TRUE( g.NumVertices() == 1 );
    added=g.addVertex("jesus");
    ASSERT_TRUE( g.NumVertices() == 2 );
    added=g.addVertex("mcdonalds");
    ASSERT_TRUE( g.NumVertices() == 3 );
    added=g.addVertex("coffee");
    ASSERT_TRUE( g.NumVertices() == 4 );
    added=g.addVertex("wingstop");
    ASSERT_TRUE( g.NumVertices() == 5 );
    added=g.addVertex("ipad");
    ASSERT_TRUE( g.NumVertices() == 6 );
    added=g.addVertex("golang");
    ASSERT_TRUE( g.NumVertices() == 7 );
    added=g.addVertex("codeine");
    ASSERT_TRUE( g.NumVertices() == 8 );
    added=g.addVertex("something1");
    added=g.addVertex("something2");
    ...
    weight = g.getWeight("something3", "something4", magnitude);
    ...
    weight = g.addEdge("insert text", "insert text", -1000);
    ASSERT_TRUE(weight);
    weight = g.getWeight("insert text", "insert text", magnitude);
    ASSERT_TRUE(magnitude == -1000);

    weight = g.addEdge("coffee", "mcdonalds", 1123);
    ASSERT_TRUE(weight);
    weight = g.getWeight("coffee", "mcdonalds", magnitude);
    ASSERT_TRUE(magnitude == 1123 );

    weight = g.addEdge("coffee", "wingstop", 1230);
    ASSERT_TRUE(weight);
    weight = g.getWeight("coffee", "wingstop", magnitude);
    ASSERT_TRUE(magnitude == 1230);

    weight = g.addEdge("coffee", "something2", -23.58934);
    ASSERT_TRUE(weight);
    weight = g.getWeight("coffee", "something2", magnitude);
    ASSERT_TRUE(magnitude == -23.58934);

    weight = g.addEdge("mcdonalds", "coffee", 12);
    ASSERT_TRUE(weight);
    weight = g.getWeight("mcdonalds", "coffee", magnitude);
    ASSERT_TRUE(magnitude == 12);

    weight = g.addEdge("wingstop", "coffee", -1000000);
    ASSERT_TRUE(weight);
    weight = g.getWeight("wingstop", "coffee", magnitude);
    ASSERT_TRUE(magnitude == -1000000);

    weight = g.addEdge("something2", "coffee", 97);
    ASSERT_TRUE(weight);
    weight = g.getWeight("something2", "coffee", magnitude);
    ASSERT_TRUE(magnitude == 97);

    weight = g.addEdge("something1", "ipad", 1230);
    ASSERT_TRUE(weight);
    weight = g.getWeight("something1", "ipad", magnitude);
    ASSERT_TRUE(magnitude == 1230);

    weight = g.addEdge("something2", "ipad", 123.5);
    ASSERT_TRUE(weight);
    weight = g.getWeight("something2", "ipad", magnitude);
    ASSERT_TRUE(magnitude == 123.5);
...
TEST(testing, test09)
{
    ...
    ASSERT_TRUE(g.NumEdges() == 1000000);
}

//
// test10
//
// This is a non-trivial test where I create a graph with
// completely negative edges and extremely small decimal
// distances.
...
TEST(testing, test10)
{
    ...
    added = g.addVertex("chicago");
    ASSERT_TRUE(added);
	added = g.addVertex("washington");
    ASSERT_TRUE(added);
	added = g.addVertex("belleville");
    ASSERT_TRUE(added);
	added = g.addVertex("elgin");
    ASSERT_TRUE(added);
	added = g.addVertex("new york city");
    ASSERT_TRUE(added);
	added = g.addVertex("memphis");
    ASSERT_TRUE(added);
	added = g.addVertex("nashville");
    ASSERT_TRUE(added);
	added = g.addVertex("albany");
    ASSERT_TRUE(added);
	added = g.addVertex("buffalo");
    ASSERT_TRUE(added);
	added = g.addVertex("baltimore");
    ASSERT_TRUE(added);
	added = g.addVertex("evanston");
    ASSERT_TRUE(added);
	added = g.addVertex("springfield");
    ASSERT_TRUE(added);
	added = g.addVertex("champaign");
    ASSERT_TRUE(added);

    added = g.addEdge("chicago", "washington", -1000);
    ASSERT_TRUE(added);
    weight = g.getWeight("chicago", "washington", magnitude);
    ASSERT_TRUE(magnitude == -1000);

    added = g.addEdge("washington", "belleville", 0.000001);
    ASSERT_TRUE(added);
    weight = g.getWeight("washington", "belleville", magnitude);
    ASSERT_TRUE(magnitude == 0.000001);

    added = g.addEdge("belleville", "elgin", -0.001);
    ASSERT_TRUE(added);
    weight = g.getWeight("belleville", "elgin", magnitude);
    ASSERT_TRUE(magnitude == -0.001);

    added = g.addEdge("elgin", "new york city", -0.123);
    ASSERT_TRUE(added);
    weight = g.getWeight("elgin", "new york city", magnitude);
    ASSERT_TRUE(magnitude == -0.123);

    added = g.addEdge("new york city", "memphis", -0.999);
    ASSERT_TRUE(added);
    weight = g.getWeight("new york city", "memphis", magnitude);
    ASSERT_TRUE(magnitude == -0.999);

    added = g.addEdge("memphis", "nashville", -890);
    ASSERT_TRUE(added);
    weight = g.getWeight("memphis", "nashville", magnitude);
    ASSERT_TRUE(magnitude == -890);

    added = g.addEdge("nashville", "albany", -1238);
    ASSERT_TRUE(added);
    weight = g.getWeight("nashville", "albany", magnitude);
    ASSERT_TRUE(magnitude == -1238);

    added = g.addEdge("albany", "buffalo", -0.0000001);
    ASSERT_TRUE(added);
    weight = g.getWeight("albany", "buffalo", magnitude);
    ASSERT_TRUE(magnitude == -0.0000001);

    added = g.addEdge("buffalo", "baltimore", 0);
    ASSERT_TRUE(added);
    weight = g.getWeight("buffalo", "baltimore", magnitude);
    ASSERT_TRUE(magnitude == 0 );

    added = g.addEdge("baltimore", "evanston", 1);
    ASSERT_TRUE(added);
    weight = g.getWeight("baltimore", "evanston", magnitude);
    ASSERT_TRUE(magnitude == 1);

    added = g.addEdge("evanston", "springfield", -0.000);
    ASSERT_TRUE(added);
    weight = g.getWeight("evanston", "springfield", magnitude);
    ASSERT_TRUE(magnitude == -0.000);

    added = g.addEdge("springfield", "champaign", -1000);
    ASSERT_TRUE(added);
    weight = g.getWeight("springfield", "champaign", magnitude);
    ASSERT_TRUE(magnitude == -1000);

    added = g.addVertex("the moon");
    ASSERT_TRUE(g.NumVertices() == 14);
}

//
// stress2
// this is a non-trivial test in which I add three million
// edges and vertices to a graph
TEST(testing, stress2)
{
    graph g;

    double large = 3000000;

    bool added;

    for ( int i = 0 ; i < large; i++ )
    {
        added = g.addVertex(to_string(i));
        ASSERT_TRUE(added);
    }

    for ( int i = 0 ; i < large; i++ )
    {
        added = g.addEdge("0", to_string(i), 1);
        ASSERT_TRUE(added);
    }

    ASSERT_TRUE(g.NumVertices() == large);
    ASSERT_TRUE(g.NumEdges() == large);
}
int main()
{
